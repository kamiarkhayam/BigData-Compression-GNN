...
def generate_sequences(data, sequence_length):
    """
    Generates input sequences and corresponding targets for a time-series problem using CNNs.
    ...
    Returns:
        tuple: 
            - inputs (np.array): Input sequences shaped as (num_sequences, 1, sequence_length, num_nodes).
            - targets (np.array): Corresponding target values shaped as (num_sequences, num_nodes).
    """
    # Extract columns with consumption data (assuming time-series data has colons in the headers, e.g., 'HH:MM' or 'YYYY-MM-DD')
    ...
    for i in range(data[consumption_columns].shape[1] - sequence_length):
        # Extract the input sequence of shape [sequence_length, num_nodes] and reshape for CNN input format
        # CNN expects [batch_size, input_channels, sequence_length, num_nodes], hence the reshape
        inputs.append(data[consumption_columns[i:i+sequence_length]].values.T.reshape((1, sequence_length, -1)))
        
        # Extract the target sequence (next time step) of shape [num_nodes]
...
def create_cnn_data(inputs, targets):
    ...
    Args:
        inputs (np.array): Input sequences (shape: [num_sequences, 1, sequence_length, num_nodes]).
        targets (np.array): Corresponding target values (shape: [num_sequences, num_nodes]).
...
def save_cnn_data(data, sequence_length, data_name, split, folder_path="generated_data"):
    """
    Saves CNN data (PyTorch Geometric Data objects) to disk in the specified folder.
...
output_folder_path = 'C:\\Users\\bmb2tn\\OneDrive - University of Virginia\\Ph.D. Projects\\Big Data\\Caltrans Traffic Data\\cnn data'
sequence_length = 4
data_name = 'caltrans_speed_data'
